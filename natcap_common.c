...
static ip_set_id_t natcap_ip_set_get_byname(struct net *net, const char *ip_set_name, struct ip_set **set)
{
	struct {
		struct nlattr nla;
...
static inline ip_set_id_t natcap_ip_set_get_byname(struct net *net, const char *ip_set_name, struct ip_set **set)
{
	return ip_set_get_byname(net, ip_set_name, set);
}
#else
static inline ip_set_id_t natcap_ip_set_get_byname(const char *ip_set_name, struct ip_set **set)
{
		...
		if (nk->magic == NATCAP_MAGIC && nk->ext_magic == (((unsigned long)ct) & 0xffffffff)) {
			if (nk->natcap_off) {
				if (nk->natflow_off && (ct->status & IPS_NATFLOW_SESSION)) {
					NATCAP_DEBUG(DEBUG_FMT_PREFIX "reuse natcap key: len=%u natcap_off=%u natflow_off=%u\n",
					              DEBUG_ARG_PREFIX, nk->len, nk->natcap_off, nk->natflow_off);
					newoff = nk->natcap_off;
				} else {
					/*
					 * The session status bit is authoritative for natcap.
					 * If it was clear but a natcap key (or a natflow key
					 * without natflow being active) is still in the private ct
        ...
        const struct nf_hook_state *state)
{
	...
	if (cone_nat_array && cone_snat_array && ntohs(UDPH(l4)->source) >= 1024 &&
		...
		unsigned int idx, css_idx;
		unsigned int max_uint_diff;
		int idx_cnt;
