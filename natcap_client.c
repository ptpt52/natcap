...
natcap_dual_out_udp:
			...
			if (!nf_ct_is_confirmed(ct)) {
				...
				if (server.ip == 0) {
					NATCAP_INFO("(CD)" DEBUG_UDP_FMT ": no server found\n", DEBUG_UDP_ARG(iph,l4));
					set_bit(IPS_NATCAP_ACK_BIT, &ct->status);
					return NF_ACCEPT;
				}
        ...
        const struct nf_hook_state *state)
{
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if (!skb_set_writable(skb, iph->ihl * 4 + sizeof(struct udphdr) + 4)) {
			return NF_ACCEPT;
		}
		iph = ip_hdr(skb);
		l4 = (void *)iph + iph->ihl * 4;
		...
		if ((NS_NATCAP_ENC & ns->n.status)) {
			if (!skb_set_writable(skb, skb->len)) {
				NATCAP_ERROR("(CPCI)" DEBUG_UDP_FMT ": skb_set_writable() failed\n", DEBUG_UDP_ARG(iph,l4));
				return NF_DROP;
			}
			iph = ip_hdr(skb);
			l4 = (void *)iph + iph->ihl * 4;
...
#define WECHAT_C_UA "User-Agent: MicroMessenger Client"
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if ((NS_NATCAP_ENC & ns->n.status)) {
			if (!skb_set_writable(skb, skb->len)) {
				NATCAP_ERROR("(CPO)" DEBUG_UDP_FMT ": skb_set_writable() failed\n", DEBUG_UDP_ARG(iph,l4));
				return NF_DROP;
			}
			iph = ip_hdr(skb);
			l4 = (void *)iph + iph->ihl * 4;
        ...
        const struct nf_hook_state *state)
{
	...
	if (master->master != ct) {
		...
		switch (iph->protocol) {
		case IPPROTO_TCP:
			NATCAP_WARN("(CPMO)" DEBUG_TCP_FMT ": bad ct[%pI4:%u->%pI4:%u %pI4:%u<-%pI4:%u] and master[%pI4:%u->%pI4:%u %pI4:%u<-%pI4:%u]\n",
			            DEBUG_TCP_ARG(iph,l4),
			            &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all),
			            &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all),
			            &ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all),
			...
			break;
		case IPPROTO_UDP:
			NATCAP_WARN("(CPMO)" DEBUG_UDP_FMT ": bad ct[%pI4:%u->%pI4:%u %pI4:%u<-%pI4:%u] and master[%pI4:%u->%pI4:%u %pI4:%u<-%pI4:%u]\n",
			            DEBUG_UDP_ARG(iph,l4),
			            &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all),
			            &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all),
			            &ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip, ntohs(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all),
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if ((NS_NATCAP_ENC & master_ns->n.status)) {
			if (!skb_set_writable(skb, skb->len)) {
				NATCAP_ERROR("(CPMO)" DEBUG_UDP_FMT ": skb_set_writable() failed\n", DEBUG_UDP_ARG(iph,l4));
				consume_skb(skb);
				return NF_ACCEPT;
			}
			iph = ip_hdr(skb);
